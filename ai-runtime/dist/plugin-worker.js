...
var permissions = /* @__PURE__ */ new Map();
...
async function loadPlugin(code) {
  ...
  const module = new vm.SourceTextModule(code, {
    ...
    initializeImportMeta(meta) {
      meta.url = "agent-plugin:bundle";
    },
    importModuleDynamically() {
      throw new Error("Plugin dynamic import is disabled");
    }
  });
  await module.link((specifier) => {
    throw new Error(`Plugin import is disabled: ${specifier}`);
  });
  await module.evaluate({ timeout: 5e3 });
  const loaded = module.namespace.default;
...
port.on("message", async (message) => {
  ...
  try {
    const handler = message.operation === "run" && typeof plugin?.run === "function" ? plugin.run.bind(plugin) : plugin?.tools?.[message.operation]?.run?.bind(plugin.tools[message.operation]);
