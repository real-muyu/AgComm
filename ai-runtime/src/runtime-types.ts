...
export type RuntimeTrustDecision = { trusted: boolean; allowUnsigned?: boolean; grants?: readonly string[] };
export interface RuntimeTrustProvider {
  authorize(request: RuntimeTrustRequest): Promise<RuntimeTrustDecision>;
}

export type RuntimeEvent =
  | { type: "flow"; event: FlowEvent }
  | { type: "tool"; trace: WorkspaceToolTrace }
  | { type: "hook"; hookId: string; workspaceId: string; stage: string; status: "start" | "complete" | "error"; elapsedMs?: number }
  | { type: "flow-hook"; hookId: string; nodeId: string; stage: string; status: "start" | "complete" | "error"; elapsedMs?: number }
  | { type: "plugin-log"; log: PluginLog };
...
type EventBase = { sequence: number; at: string };
export type AiStreamEvent =
  | EventBase & { type: "run-start"; packageHash: string; projectName: string; mode: AiStreamMode }
  | EventBase & { type: "model-start"; context: ModelInvocationContext }
  | EventBase & { type: "model-event"; context: ModelInvocationContext; event: ModelEvent }
  | EventBase & { type: "model-complete"; context: ModelInvocationContext; hasToolCalls: boolean; contentLength: number }
  | EventBase & { type: "runtime-event"; event: RuntimeEvent }
  | EventBase & { type: "output-delta"; text: string; nodeId?: string }
  | EventBase & { type: "result"; result: AiRunResult }
  | EventBase & { type: "error"; error: { code: string; name: string; message: string } };
export type AiStreamEventInput = AiStreamEvent extends infer Event
  ? Event extends AiStreamEvent ? Omit<Event, "sequence" | "at"> : never
  : never;
...
export type KnowledgeImportOptions = { scope: KnowledgeScope; signal?: AbortSignal; onProgress?: (progress: KnowledgeProgress) => void };
...
export interface AiSessionHandle {
  ...
  streamTurn(input: string, options: SessionRunOptions & { mode: "text" }): AiRunStream<string>;
  streamTurn(input: string, options: SessionRunOptions & { mode: "events" }): AiRunStream<AiStreamEvent>;
  streamTurn(input: string, options?: SessionRunOptions & { mode?: AiStreamMode }): AiRunStream<string | AiStreamEvent>;
...
export type AiAppInfo = {
  ...
  packageHash: string;
  nodes: ReadonlyArray<{ id: string; title: string; type: string }>;
  ...
  background?: {
    ...
    triggers: ReadonlyArray<{ id: string; type: "heartbeat" | "cron"; schedule: string }>;
  };
};

export interface AiRuntime {
  runAiFile(pathOrBytes: string | Uint8Array | ArrayBuffer, options?: RunAiOptions): Promise<AiRunResult>;
  streamAiFile(pathOrBytes: string | Uint8Array | ArrayBuffer, options: StreamRunOptions & { mode: "text" }): Promise<AiRunStream<string>>;
  streamAiFile(pathOrBytes: string | Uint8Array | ArrayBuffer, options: StreamRunOptions & { mode: "events" }): Promise<AiRunStream<AiStreamEvent>>;
