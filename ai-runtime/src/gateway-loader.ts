...
export type GatewayClientLike = {
  ...
  install(path: string, options?: { enabled?: boolean; webhook?: { url: string; secret?: string } }): Promise<unknown>;
  ...
  startRunNow(id: string, triggerId: string): Promise<{ runId: string }>;
  watchRun(id: string, runId: string, options: { mode: "text"; signal?: AbortSignal }): Promise<AsyncIterable<{ sequence: number; value: unknown }> & {
    completion: Promise<{ status: string; elapsedMs?: number; error?: string }>;
  }>;
...
export async function connectRuntimeGateway(options?: { root?: string }) {
  return (await loadGatewayModule()).connectRuntimeGateway(options);
}
